...
class DBHelper extends SQLiteOpenHelper
{
    ...
    public void onCreate(SQLiteDatabase db)
    {
       ...
       // create table for transactions
        db.execSQL("create table " + TransactionDbIds.TABLE + "(" +
                ...
                TransactionDbIds.PREMIUM + " TEXT," +
                TransactionDbIds.BUDGET + " TEXT," +
                TransactionDbIds.COMMISSION + " REAL not null," +
    ...
    public List<Budget> getBudgets(long startDateMs, long endDateMs)
    {
        ...
        final String TRANS_COMMISSION = TransactionDbIds.TABLE + "." + TransactionDbIds.COMMISSION;
        ...
        Cursor data = db.rawQuery("select " + BUDGET_ID + ", " + BUDGET_MAX + ", " +
                "(select total(" + TRANS_COMMISSION + ") from " + TransactionDbIds.TABLE + " where " +
    ...
    public Budget getBlankBudget(long startDateMs, long endDateMs)
    {
        ...
        final String TRANS_VALUE = TransactionDbIds.TABLE + "." + TransactionDbIds.COMMISSION;
        final String TRANS_TYPE = TransactionDbIds.TABLE + "." + TransactionDbIds.TYPE;
        final String TRANS_DATE = TransactionDbIds.TABLE + "." + TransactionDbIds.DATE;
        final String TRANS_BUDGET = TransactionDbIds.TABLE + "." + TransactionDbIds.BUDGET;
     ...
     * false otherwise
     */
    public boolean insertTransaction(final int type, final String description, final String premium, final String budget,
                                 final double commission, final String note, final long dateInMs, final String receipt)
    {
        ContentValues contentValues = new ContentValues();
        contentValues.put(TransactionDbIds.TYPE, type);
        contentValues.put(TransactionDbIds.DESCRIPTION, description);
        contentValues.put(TransactionDbIds.PREMIUM, premium);
        contentValues.put(TransactionDbIds.BUDGET, budget);
        contentValues.put(TransactionDbIds.COMMISSION, commission);
     ...
     * false otherwise
     */
    public boolean insertTransaction(SQLiteDatabase writableDb, final int id, final int type, final String description, final String premium, final String budget,
                                     final double commission, final String note, final long dateInMs, final String receipt)
    {
        ContentValues contentValues = new ContentValues();
    ...
    public boolean updateTransaction(final int id, final int type, final String description,
                                     final String premium, final String budget, final double commission,
    ...
    public Cursor getTransactions(int type, String budget, String search, Long startDateMs, Long endDateMs)
    {
        ...
        if(search != null)
        {
            ...
            String [] items = new String[]{TransactionDbIds.DESCRIPTION, TransactionDbIds.PREMIUM,
                    TransactionDbIds.COMMISSION, TransactionDbIds.NOTE};
    ...
    public ArrayList<Transaction> getTransactionsWithReceipts()
    {
        ArrayList<Transaction> transactionArrayList = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor =  db.rawQuery("select * from " + TransactionDbIds.TABLE,null);
        while(cursor.moveToNext()){
            ...
            Transaction transaction = new Transaction(id, type, toBlankIfNull(description), toBlankIfNull(premium),
    ...
    private String toBlankIfNull(final String string)
    {
        if(string != null)
        {
            return string;
