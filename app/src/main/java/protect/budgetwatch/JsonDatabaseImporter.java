...
public class JsonDatabaseImporter implements DatabaseImporter
{
    public void importData(Context context, DBHelper db, InputStream input, ImportExportProgressUpdater updater) throws IOException, FormatException, InterruptedException
    {
        ...
        try
        {
            ...
            while(parser.hasNext())
            {
                ...
                while(parser.hasNext())
                {
                    ...
                    switch (itemName)
                    {
                        ...
                        case DBHelper.TransactionDbIds.DESCRIPTION:
                            ...
                            break;
                        case DBHelper.TransactionDbIds.PREMIUM:
                            premium = parser.nextString();
                            break;
                        case DBHelper.TransactionDbIds.BUDGET:
                            budget = parser.nextString();
                            break;
                        case DBHelper.TransactionDbIds.COMMISSION:
                            commission = parser.nextDouble();
                            break;
                ...
                {
                    case "BUDGET":
                        importBudget(database, db, name, commission);
                        break;
                    case "EXPENSE":
                    case "REVENUE":
                        importTransaction(context, database, db, id, type, description, premium, budget, commission, note, dateMs, receiptFilename);
                        break;
                    default:
    ...
    private void importTransaction(Context context, SQLiteDatabase database, DBHelper helper,
                                   Integer id, String typeStr, String description, String premium,
                                   String budget, Double commission, String note, Long dateMs,
            ...
            throws FormatException
    {
        ...
        if(id == null || budget == null || commission == null || dateMs == null)
        {
        ...
        helper.insertTransaction(database, id, type, description, premium, budget, commission, note, dateMs, receipt);
    }
     ...
     * session.
     */
    private void importBudget(SQLiteDatabase database, DBHelper helper, String name, Double commission)
            throws FormatException
    {
        // Check that both fields exist
        // Ensure that the required data exists
        if(name == null || commission == null)
        {
            throw new FormatException("Missing required data in JSON record");
        }

        helper.insertBudget(database, name, commission.intValue());
    }
}
