        ...
        try
        {
            ...
            })
            {
                while (cursor.moveToNext())
                {
                    ...
                    writer.name(DBHelper.TransactionDbIds.PREMIUM).value(transaction.premium);
                    writer.name(DBHelper.TransactionDbIds.BUDGET).value(transaction.budget);
                    writer.name(DBHelper.TransactionDbIds.COMMISSION).value(transaction.commission);
            ...
            for (String budgetName : budgetNames)
            {
                ...
                writer.name(DBHelper.TransactionDbIds.COMMISSION).value((double)budget.max);
